...
public class Branch3 : MonoBehaviour
{
    public int id;
    ...
    //Material material;
    public MeshGenerator10 meshGenerator;
    ...
    {
        if (curGen >= meshGenerator.generations)
        { 
            meshGenerator.EndSegment(id);
            return; 
        }
        ...
        switch (curRule)
        {
            case 'u': meshGenerator.Bend(id, 1); break;
            case 'd': meshGenerator.Bend(id, -1); break;
            case 'r': meshGenerator.Rotation(id, 1); break;
            case 's': meshGenerator.Rotation(id, -1); break;
