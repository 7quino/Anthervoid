...
public class PlantGenerator : MonoBehaviour
{
    ...
    IEnumerator PositionRotatePlant()
    {
        ...
        float timeEnd = 50 + Time.deltaTime;
        ...
        {
            transform.position = Vector3.Lerp(startPos, endPos, timeElapsed/timeEnd);
            transform.rotation = Quaternion.Lerp(Quaternion.Euler(Vector3.zero), Quaternion.Euler(rotation), timeElapsed / timeEnd);
    ...
    void CreateFirstBranch(int id)
    {
        ...
        branches.Add(newBranch.GetComponent<Branch>());
        branches[id].id = id;
        branches[id].meshGenerator = gameObject.GetComponent<PlantGenerator>();
    ...
    public void CreateBaseShape(int id)
    {
        if (id == 0)
        {
            ...
            for (int i = 0; i < sides; i++)
            {
                ...
                branches[id].lerpEndPos.Add(Quaternion.AngleAxis(sideAngle * i, Vector3.up) * new Vector3(branches[id].radiusV.x, 0, branches[id].radiusV.z));                
            }

            StartCoroutine(LerpGrowthRadiusBaseShape(0, 0));
        }
        ...
        if (id == 1)
        {
            ...
            StartCoroutine(LerpGrowthRadiusBaseShape(1, 0));
        }
    }


    public IEnumerator LerpGrowthRadiusBaseShape(int id, int vertIndex)
    {
        float timeElapsed = Time.deltaTime;
        float timeEnd = growthSpeed * growthMultiplierRadius + Time.deltaTime;
        List<Vector3> startPos = new List<Vector3>();
        List<Vector3> endPos = new List<Vector3>();

        Vector3 avgPoint = new Vector3();
        foreach (Vector3 point in branches[id].lerpEndPos) avgPoint += point;
        avgPoint /= branches[id].lerpEndPos.Count;

        for (int i = 0; i < sides; i++)
        {
            startPos.Add(avgPoint);
    ...
    public void AddBranch(int id, LSystemBranch branch)
    {
        ...
        branches[newId].meshGenerator = gameObject.GetComponent<PlantGenerator>();
