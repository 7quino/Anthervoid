...
public class MeshGenerator8 : MonoBehaviour
{
    //mesh
    ...
    void CreateFirstBranch()
    {
        ...
        branches.Add(newBranch.GetComponent<Branch1>());
        branches[0].rules.AddRange(ruleListA);
        branches[0].id = 0;
        branches[0].firstInstance = true;
        branches[0].meshGenerator = gameObject.GetComponent<MeshGenerator8>();
    }
    ...
    {
        branches[id].curRotation = (branches[id].curRotation + rotation) % 360;
        branches[id].bendAxis = Quaternion.AngleAxis(branches[id].curRotation, Vector3.up) * Vector3.one;
        branches[id].RunNextRule();
    }
    ...
    {
        branches[id].centerVector = Quaternion.AngleAxis(branches[id].curBend, branches[id].bendAxis) * branches[id].lenghtV;
        ...
        for (int i = 0; i < sides;  i++)
        {
            branches[id].lerpEndPos.Add((Quaternion.AngleAxis(sideAngle * i -branches[id].curBend, branches[id].centerVector) * branches[id].radiusV) + branches[id].spaceVector);
        }
    ...
    public void RuleABCD(int id, LSystemBranch branch)
    {
        ...
        branches[newId].meshGenerator = gameObject.GetComponent<MeshGenerator8>();
